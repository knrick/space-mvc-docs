...
```
After the user is redirected, you may display the flashed message from the <a href="/docs/5.7/session">session</a>. For example, using <a href="/docs/5.7/blade">Blade syntax</a>:
```php
@if (session('status'))
